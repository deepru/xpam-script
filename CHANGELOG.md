...
- **Сайты-обманки теперь «живые» — у каждого из 10 архетипов уместное движение.** Раньше двигалась
  ...
  мигает курсор в конце вывода; у карточки API (`financial`, `soft`, `midnight`, `slate`) линия графика
  сама прочерчивается медленным циклом; у `editorial` под ключевым словом заголовка прочерчивается
  тонкая акцентная линия; у схемы (`clean`, `aurora`) — прежний бегущий поток. Только CSS, без JS; при
