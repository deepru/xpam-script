...
<!-- Historical entries describe commands that existed in past releases. That is what a changelog is
     for, so the removed-command gate in make-release.sh does not apply to this file. -->

Что сюда попадает: всё, что изменилось в продукте, по одному-трём предложениям на пункт. Подробности
диагностики и доказательства живут в сообщениях коммитов. В release notes уходит **не всё отсюда**, а
только то, что меняет что-то для пользователя, — правила в `AGENTS.md`.

## v1.4.0

Запасной транспорт VLESS на отдельном домене, полностью автоматический WARP, уникальные
сайты-маскировки на каждой установке, переработанное меню и еженедельные обновления системы с
...
### Документация
- **Руководство пользователя врало про нумерацию пунктов установки.** Главы 5–7 называли первый шаг
  «пунктом 0», а второй — «пунктом 1», хотя в меню `1` — это SSH-безопасность, `2` — установка, а `0` —
  выход. Пользователь, буквально следующий руководству на самом первом экране, выходил из установщика.
  Шаги переименованы в «Шаг 1» и «Шаг 2» — теперь номер шага совпадает с номером пункта меню, и
  рассинхронизироваться им больше нечем. Заодно описан двухзаходный характер установки (подготовка →
  перезагрузка → достройка), который раньше упоминался только вскользь в предупреждении.
- **Руководство и `docs/MAINTENANCE.md` описывали поведение обслуживания до этого релиза.** Оба
  утверждали, что сервер «сам не перезагружается» и что о необходимости перезагрузки придёт сообщение.
  С переходом на режим `auto` всё наоборот: обновления ставятся вместе с ядром, сервер при
  необходимости перезагружается сам, а сообщение приходит только при сбое. Это самое неожиданное для
  оператора действие продукта, и документация говорила о нём ровно противоположное. Описано как есть,
  вместе с одноразовым проходом после перезагрузки и способом это отключить.
- **Задокументирован реальный способ отключить автоперезагрузку.** Очевидное значение
  `XPAM_MAINT_APT_MODE=security` не работает: миграция возвращает его в `auto` при каждом `repair`.
  Нужное значение — `upgrade`; в руководстве теперь таблица режимов с этим предупреждением.
- **Убраны эмодзи из всех 25 заголовков руководства** и из оглавления: набор технической документации,
  а не пост в канале.
- **Единая терминология.** Одна и та же функция называлась в документации пятью способами («запасной
  способ подключения», «запасной вход», «запасное подключение», «альтернативный транспорт», «запасной
  транспорт»). Везде оставлен **запасной транспорт** — согласуется с README и с пунктом меню.
- **Новый раздел 9.1 «Команды, которых больше нет».** Удалённые в этом релизе `-status`, `-netdiag`,
  `-vless`, `-tg` и флаг `--show-secrets` перечислены вместе с тем, что вместо них, — единственное
  изменение релиза, способное сломать чужие заметки и скрипты.
- **Шаблон issue на GitHub просил вычистить вывод `<prefix>-links --show-secrets`** — флага, удалённого
  в этом же релизе. Первое, что видел человек, открывая баг-репорт, ссылалось на несуществующую команду.
- Описание `repair --full` дополнено предупреждением про запасной транспорт (соответствует поведению,
  добавленному в `bb734b3`), описание обновления и глубокой проверки переписано без внутреннего
  жаргона (`staging`, `invariants`, `post-update`, `provider-specific quirks`), из `docs/MAINTENANCE.md`
  убран список требований к разработчику («The updater must…») в пользу описания того, что происходит.
- Исправлена задвоенная строка в чек-листе главы 25.

### Читаемость вывода диагностики
- **`<prefix>-netdiag` больше не выглядит как команда, которая ничего не делает.** Она собирает
  подробный срез сетевого состояния, но на экран печатала только путь к файлу — то есть всю пользу
  приходилось искать вручную. Теперь она показывает главное сразу: интерфейс и адрес, шлюз,
  DNS-серверы, состояние `networking`, число упавших юнитов, результат проверки DNS и число замечаний
  в отчёте. Полный отчёт по-прежнему пишется в файл — его и передают, когда нужна вся картина.
- **Проверка портов перестала быть стеной из `ss`.** Печатались три сырых дампа с колонками
  Recv-Q/Send-Q, одинаковым столбцом собеседника и хвостом `users:(("proc",pid=…,fd=…))` на каждый
  сокет — шире терминала и с пробелами до конца строки. Теперь это две группы, публичные и локальные,
  где у каждого порта написано, чей он и зачем — включая служебные порты Xray и MTG, которые раньше
  просто висели без объяснения. Имена процессов разворачиваются: `ss` обрезает их до 15 символов и
  показывал `xray-linux-amd6`.
- **Проверка сертификатов сжата с 53 строк до 8.** На каждый из пяти эндпоинтов печатался блок
  subject/issuer/notBefore/notAfter/SAN, хотя на профиле с корневым доменом четыре из пяти
  эндпоинтов отдают **один и тот же** сертификат — один и тот же текст четыре раза подряд. Теперь
  строка на эндпоинт: что проверяли, какой SNI, результат и сколько дней осталось. При проблеме
  строка показывает причину, и рядом остаётся явная строка `FAIL:` с подробностями.
- Мелочи там же: `Аптайм: …` вместо голой строки `uptime`, размер снапшота вместо сырого `ls -lh`,
  правила UFW без висящих пробелов.
- **Проверено сравнением вердиктов.** Набор строк `OK:`/`FAIL:`/`WARN:`/`INFO:` до и после правок
  совпадает дословно, кроме десяти построчных `OK:` по сертификатам, которые заменены строками
  таблицы. Ни одна проверка, ни один счётчик и ни один код возврата не изменены — правки касаются
  только печати. Вывод глубокой проверки: 404 строки → 359.

### Fixes — самообновление
- **Самообновление было сломано в обе стороны, и это чуть не остановило переход на 1.4.0.** Апдейтер
  версии 1.3.9 после применения обновления проверяет, что на месте команды `health`, `links`, `repair`
  и `netdiag`. Этот список удалить из уже установленных копий нельзя — он выполняется старым кодом,
  который к тому моменту уже загружен в оболочку. Мы же в этом релизе `<prefix>-netdiag` удалили, и
  проверка не находила файл: обновление объявлялось неудачным и откатывалось. Отката это не портило —
  сервер возвращался к рабочей версии с неизменными ссылками, — но подняться до 1.4.0 через меню было
  невозможно. Хуже того, тот же список остался и в постчеке самого 1.4.0, то есть сервер, доехавший до
  этой версии, не смог бы обновиться уже никогда.
  **Исправлено двумя правками:** `<prefix>-netdiag` возвращён (это тонкая обёртка над тем же
  действием, что и пункт меню, — она ничего не стоит и держит путь обновления открытым), а из постчека
  убрано требование, которого кит не гарантирует. Удалёнными остаются `-status`, `-vless`, `-tg` и
  флаг `--show-secrets`: их старый постчек не проверял.
- **Новый гейт: список команд в постчеке апдейтера обязан быть подмножеством того, что кит реально
  создаёт.** Обе половины дефекта по отдельности выглядели корректно — ошибкой была только их пара,
  поэтому не поймал никто. Теперь сборка сверяет `xpam_update_postcheck` с функциями
  `write_*_launcher` и падает с указанием недостающей команды. Проверено воспроизведением исходного
  дефекта. (Первая версия гейта, к слову, не сработала: она искала имя функции по всему файлу и
  удовлетворялась упоминанием в комментарии — исправлено на поиск только определений.)

### Документация: правила и гейт
- **Правила оформления публичных документов записаны в `AGENTS.md`.** Каждое из них было нарушено
  минимум один раз, и результат доезжал до пользователя: смешение читателей в одном файле (требования
  к разработчику в пользовательской документации), пять имён у одной функции, факты по памяти вместо
  кода, эмодзи в заголовках, один и тот же факт в восьми файлах. Там же зафиксировано разделение
  CHANGELOG и release notes: notes — строгое подмножество, отобранное вопросом «меняет ли это
  что-нибудь для того, кто пользуется продуктом», и в них никогда не попадают исправления багов,
  которых не было ни в одном релизе, работа над сборкой и версии третьих компонентов.
- **Новый гейт: публичные документы не должны упоминать удалённые команды.** Документацию не читала
  ни одна проверка, поэтому шаблон issue на GitHub спокойно просил вычистить вывод
  `--show-secrets` — флага, удалённого в этом же релизе. Гейт проверяет `*.md` и `.github/` на имена
  удалённых команд; файлы, которые обязаны их называть (changelog, руководство, release notes),
  объявляют это маркером `xpam-lint: legacy-commands-documented`. Проверено срабатыванием: возврат
  исходного дефекта в шаблон issue роняет сборку с указанием файла и строки.

### Документация: публичные страницы
- **Русская главная вела в английскую документацию.** Все ссылки в разделе «Документация» были
  подписаны по-русски, а открывался английский текст: единственным русским документом было
  руководство. Раздел перестроен вокруг руководства — теперь это ссылки на его главы, — а английский
  набор помечен как краткий справочник.
- **Список секретов больше не переписан в восьми местах.** Он был в `SECURITY.md`,
  `docs/SECURITY_MODEL.md`, `TESTING.md`, шаблоне issue, трижды в `docs/TROUBLESHOOTING.md`, в
  `docs/RELEASE_PROCESS.md` и кусками ещё в двух файлах — с расхождениями, потому что часть копий
  отстала от удаления `--show-secrets`. Единственный источник — `SECURITY.md`, остальные ссылаются.
- **`docs/SECURITY_MODEL.md` переписан в модель, а не в повтор списка секретов**: что открыто наружу,
  что закрыто, где границы доверия (у оператора root; ссылка подключения — это доступ; Exit-сервер
  чужой; 3x-ui остаётся универсальной панелью).
- **`TESTING.md` больше не публикует матрицу «что мы проверяли».** Это запрещено правилами в
  `AGENTS.md` и вдобавок стареет молча: в списке значилось еженедельное обслуживание, ветка
  перезагрузки в котором на тот момент не была проверена ни разу. Файл теперь про то, как пользователь
  проверяет свой сервер.
- **Удалены `docs/PROFILES.md` и `docs/RELEASE_PROCESS.md`.** Первый не был подключён ни к одной
  странице и повторял `ARCHITECTURE.md`; второй — чеклист мейнтейнера, опубликованный в
  пользовательской документации (его место в `handoff/`). Удалён и
  `RELEASE_NOTES_v1.3.9_RU.md`: актуальными остаются notes только текущей версии.
- **Из пользовательской документации убраны инструкции для разработчика**: «Use placeholders in
  examples» в `INSTALLATION.md` и `SITES.md`, «The updater must…» в `MAINTENANCE.md`, раздел
  «Terminology» в `docs/README.md`. В `INSTALLATION.md` добавлено то, чего не хватало, — что установка
  идёт двумя пунктами меню и что пункт 2 выполняется дважды, с перезагрузкой между заходами.
- **В README добавлен раздел «Что это не решает»** (один оператор на установку, не биллинг-панель, не
  обещание невидимости, Exit-сервер готовит пользователь) и явно сказано, что bootstrap сверяет
  контрольную сумму с файлом `.sha256` из релиза.

### Release notes v1.4.0 — переписаны
- Из раздела исправлений убраны **пять пунктов из шести**: все они чинили дефекты, появившиеся и
  исправленные внутри этого же цикла разработки, то есть не существовавшие ни в одном выпущенном
  релизе. Для пользователя это не исправления, а перечисление того, чего он не переживал; вдобавок
  такой список заставляет новую функцию выглядеть сырой.
- Добавлены **три настоящих исправления**, которых в notes не было: WAL-безопасные копии базы,
  перевыпуск данных Telegram-клиента при каждом `repair` и предупреждение `repair --full`.
- Добавлен раздел **«Изменения в командах»** — единственное в релизе, что способно сломать чужие
  скрипты и привычки, и раньше подавалось как улучшение внутри рассказа про меню.
- Автоматическая перезагрузка вынесена в отдельный раздел и названа прямо, вместе со способом её
  отключить: это самое неожиданное действие продукта.
- Добавлены установка с нуля и команда самостоятельной проверки контрольной суммы — тело релиза это
  ещё и первая страница, которую видит новый человек.
- Убраны эмодзи из заголовков, «Большой релиз на базе v1.3.9», обещания вроде «включается за одну
  минуту», разнобой в терминологии и повторы про неизменность ссылок.
...
### Fixes
- **WARP снова настраивается на свежеустановленном сервере.** На чистой установке 3x-ui ещё не создаёт
  в своей базе строку с шаблоном конфигурации Xray — она появляется только после первого сохранения
  настроек. WARP же в эту строку дописывает исходящее подключение, поэтому на новом сервере настройка
  обрывалась: аккаунт Cloudflare регистрировался, а исходящее не создавалось, и в логе появлялось
  «setting xrayTemplateConfig не найден в 3x-ui DB». Дальше и смена WARP-адреса не работала, потому
  что нечего было менять. DoubleHop с этим сталкивался давно и умеет создавать нужную строку сам —
  теперь тем же способом пользуется и WARP, в обоих путях: и при первичной настройке, и при повторной
  проверке (пункт «только проверить и привести к настройкам»), который вызывается ещё и из repair.
  Повторный вызов безопасен: если строка уже есть, ничего не меняется. Найдено на чистой установке —
  на серверах, где WARP когда-то настраивали, строка уже существовала, и дефект не проявлялся.
- **Полное восстановление предупреждает про запасной транспорт.** `repair --full` восстанавливает
  базу 3x-ui, но не файл настроек XPAM — так и задумано, иначе команда молча откатывала бы и то, что
  оператор менял осознанно. Из-за этого, если снапшот сняли до включения запасного транспорта, после
  восстановления получалось расхождение: в настройках транспорт включён, а его подключения в базе нет,
  и проверка состояния падала без понятной причины. Теперь: перед подтверждением восстановления
  выводится предупреждение, что в этом снапшоте запасного транспорта нет и его нужно будет включить
  заново (ссылка обновится), а основные VLESS/Telegram-подключения не затрагиваются; а если
  расхождение всё же возникло, проверка состояния прямо объясняет причину и что сделать, вместо
  сухого «инбаунд не слушает порт». Подключение при этом не пересоздаётся автоматически: это выдало бы
  новый идентификатор клиента, то есть молча сменило бы уже выданную ссылку.
