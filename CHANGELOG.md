...
- **Поддерживаемые системы больше не привязаны к номерам версий.** Правило для документации
  требовало указывать «Ubuntu и Debian, включая Debian 13 и Ubuntu 26.04». Проверка в коде смотрит
  только на сам дистрибутив и не проверяет версию вообще — ни минимальную, ни максимальную (и
  диагностика намеренно не привязана к версии, чтобы новые выпуски не давали ложных ошибок). То есть
  конкретные номера занижали реальную совместимость и вдобавок портятся со временем: пара версий,
  которая сегодня читается как «свежая», после выхода следующих выпусков читается как «заброшено» —
  хотя скрипт продолжает работать. Теперь платформы указываются как Ubuntu и Debian без версий, один
  раз, как свойство продукта.
