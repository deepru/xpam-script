...
XPAM Script использует safe DNS behavior:
...
XPAM Script нормализует `/etc/hosts` так, чтобы hostname резолвился корректно, а XPAM-managed домены не были привязаны к `127.0.0.1` или `127.0.1.1`.
...
Текущая проверочная матрица ведётся в [TESTING.md](TESTING.md).
