...
mtproto_alexbers_harden_config(){
...
mtproto_backend_assert_alexbers_runtime(){
  uses_mtproto || return 0
  local backend
  ...
  case "$backend" in
    alexbers) return 0 ;;
    none) return 0 ;;
    3xui-mtg|teleproxy) fail "$(mtproto_backend_reserved_message "$backend")" ;;
...
mtproto_backend_install(){
  ...
  case "$backend" in
    alexbers)
      mtproto_3xui_mtg_delete_managed_if_present || true
      mtproto_alexbers_install "$@"
      ;;
    3xui-mtg)
      mtproto_3xui_mtg_install "$@"
      ;;
    teleproxy)
      fail "$(mtproto_backend_reserved_message "$backend")"
      ;;
    none)
      return 0
      ;;
...
# mtproto_backend_install so backend ownership stays explicit.
install_mtproto(){ mtproto_backend_install "$@"; }

mtproto_backend_harden_config(){
  uses_mtproto || return 0
  mtproto_backend_assert_alexbers_runtime
  mtproto_alexbers_harden_config "$@"
}
...
mtproto_harden_config(){ mtproto_backend_harden_config "$@"; }

mtproto_backend_rewrite_notes(){
  uses_mtproto || return 0
  mtproto_backend_assert_alexbers_runtime
  mtproto_python_update rewrite-notes "$@"
}
...
mtproto_backend_try_restart_runtime(){
  uses_mtproto || return 0
  local backend
  backend="$(mtproto_backend_effective)"
  case "$backend" in
    alexbers)
      mtproto_3xui_mtg_delete_managed_if_present || true
      systemctl try-restart mtprotoproxy
      ;;
    3xui-mtg)
      mtproto_3xui_mtg_repair_if_needed || true
      ;;
    teleproxy)
      fail "$(mtproto_backend_reserved_message "$backend")"
      ;;
...
mtproto_backend_repair_after_update(){
  ...
  case "$backend" in
    alexbers)
      mtproto_3xui_mtg_delete_managed_if_present || true
      mtproto_alexbers_harden_config || true
      mtproto_python_update rewrite-notes >/dev/null 2>&1 || true
...
print_tg_summary(){
  ...
  echo "============================================================"
  echo
  echo "Эта команда по умолчанию НЕ печатает Telegram ссылки, чтобы случайно не раскрыть доступ."
  echo
  echo "Файлы с Telegram данными:"
  [[ -f "$mt_note" ]] && echo "  Основная ссылка:       $mt_note"
  [[ -f "$mt_users_note" ]] && echo "  Пользователи:          $mt_users_note"
  echo
  echo "Показать Telegram ссылки на экран:"
...
stage_tg_direct(){
  ...
  if ! mtproto_backend_is_alexbers; then
    fail "${SERVER_PREFIX}-tg is not used for 3xui-mtg. Use: sudo ${SERVER_PREFIX}-links"
  fi
  mtproto_require
  case "${1:-}" in
    --show)
...
stage_mtproto_users_menu(){
  ...
  validate_inputs
  if ! mtproto_backend_is_alexbers; then
    fail "Telegram uses one XPAM-managed link; use: sudo ${SERVER_PREFIX}-links"
  fi
  mtproto_require
  echo "MTProto пользователи"
